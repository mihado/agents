...
import { scanLines, scanFile, fingerprint, walk } from "./scanner.js";
...
describe("scanLines", () => {
  ...
  it("detects code risk patterns in scripts", () => {
    const lines = [
      'import { execSync } from "node:child_process";',
      "const x = 1;",
      'eval("something")',
    ];
    const findings = scanLines(lines, ".mjs");
    expect(findings).toHaveLength(2);
    expect(findings[0].label).toBe("child_process");
    expect(findings[1].label).toBe("eval()");
  });
  ...
  it("applies only injection patterns to prose, not code patterns", () => {
    const findings = scanLines(['eval("something")'], ".md");
    expect(findings).toHaveLength(0);
  });

  it("applies both injection and code patterns to code files", () => {
  ...
  it("detects subprocess patterns", () => {
    const findings = scanLines(["subprocess.run(['ls', '-la'])"], ".py");
    expect(findings).toHaveLength(1);
    expect(findings[0].label).toBe("subprocess");
  });
});
