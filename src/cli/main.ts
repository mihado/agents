...
const root = (): string => path.resolve(fileURLToPath(import.meta.url), "../../..");
...
vendor
  ...
  .action((opts: { accept?: boolean; showSuppressed?: boolean; skillspector?: boolean }) => {
    const reviewOpts: ReviewOptions = {
      root: root(),
      accept: opts.accept,
      showSuppressed: opts.showSuppressed,
      withSkillspector: opts.skillspector,
    };
...
vendor
  ...
  .action((opts: { json?: boolean; accept?: boolean; skillspector?: boolean }) => {
