...
3. Require `<target_invocation_dir>/.agent-contexts/work/<work-id>/` to be absent, then write the request at the absolute `<target_invocation_dir>/.agent-contexts/work/<work-id>/delegated-01.md` path. A target work-directory collision blocks the handoff. This is the entire cross-repository publication: do not write a target Brief, Plan, attempt, or `<target_invocation_dir>/.agent-contexts/active.md`.
...
3. If local work is active, require the user's selection before changing it. Otherwise create the local Brief by preserving the request's outcome, acceptance criteria, hard constraints, settled decisions, repository scope, and expected evidence; record the request and consulted context paths in `source_contexts`; update only this conductor's absolute `active.md` path with `delivery_mode: approval-required`; then write `status: activated` and `activated_at` into the request. Set `delivery_mode: autonomous` only when the same activation instruction explicitly grants autonomous delivery. A failed activation before that final write leaves no activated request.
