---
name: wf-execution
description: Executes settled implementation units with bounded authority and evidence handoffs. Use when applying an approved execution plan.
---
...
Completion criterion: the approved unit is complete or returns `BLOCKED` or `NEEDS_CONTEXT`; the handoff records changed scope, observed proof, retry safety, and every material supporting skill used.
...
- Proof observed:
  - <unit> — <evidence strategy, command/result, runtime observation, or pre/post state>
- No-test exceptions:
  - <unit> — <why automated proof was unsuitable and replacement evidence>
...
- Return the handoff to the conductor; the conductor writes workflow artifacts.
