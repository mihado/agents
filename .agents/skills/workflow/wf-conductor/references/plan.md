...
5. Every `advanced` AC has a named evidence expectation. When Execution Slices split the Plan across multiple named slices and an AC's evidence is not complete until more than one lands, its entry names every contributing slice and the portion each owns, rather than declaring `advanced` against the whole Plan.
